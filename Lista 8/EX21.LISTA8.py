...
resultados = [         
    ...
    {"ra": "2024003", "cod": "HIS103", "sem": 1, "ano": 2026, "nota": 6.8, "freq": 80},
    {"ra": "2024003", "cod": "QUI104", "sem": 2, "ano": 2026, "nota": 8.0, "freq": 85}
]

def alunocommaiornota(alunos,disciplinas,resultados):

    if len(disciplinas) == 0 or len(alunos) == 0 or len(resultados) == 0:
        return None
    
    i = 0 
    maiornota = resultados[0]["nota"]
    maiorra = resultados[0]["ra"]

    while i < len(resultados):
        if resultados[i]["nota"] > maiornota:
            maiornota = resultados[i]["nota"]
            maiorra = resultados[i]["ra"]
        i += 1
    ...
    while j < len(alunos):
        if alunos[j]["ra"] == maiorra:
            nomealuno = alunos[j]["nome"]
        j += 1

    return nomealuno, maiorra, maiornota

print(alunocommaiornota(alunos, disciplinas, resultados))
